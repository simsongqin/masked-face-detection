...
def eval(model, model_path=None, is_gray=False):
    predicts = []
    check_points = torch.load(model_path, map_location='cuda:0')
    ...
    root = '/home/FYP/sims0038/lfw-deepfunneled_cropped_112x96/'
    with open('LFW_pairs.txt') as f:
        pairs_lines = f.readlines()[1:]
    
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))  # range [0.0, 1.0] -> [-1.$
    ])
    ...
    for i in range(len(pairs_lines)):
        ...
        if len(p) == 3:
            sameflag = 1  # two pics of same person
            name1 = '/' + p[0] + '_' + '{:04}'.format(int(p[1])) +'_0_align_.jpg'
            name2 = '/' + p[0] + '_' + '{:04}'.format(int(p[2])) +'_0_align_.jpg'


        elif len(p) == 4:   # jesse 1 emma 2 not the same person
            sameflag = 0
            name1 = '/' + p[0] + '_' + '{:04}'.format(int(p[1])) +'_0_align_.jpg'
            name2 = '/' + p[2] + '_' + '{:04}'.format(int(p[3])) +'_0_align_.jpg'
        ...
        with torch.no_grad():
            ...
            img1_ = img1_.unsqueeze(0).cuda()
            img2 = img2.unsqueeze(0).cuda()
            img2_ = img2_.unsqueeze(0).cuda()

        feature1_1 = model(img1)
        feature1_2 = model(img1_)
        feature2_1 = model(img2)
        feature2_2 = model(img2_)

        f1 = torch.cat((feature1_1,feature1_2), 1).data[0]
        f2 = torch.cat((feature2_1,feature2_2), 1).data[0]
    ...
    predicts = map(lambda line: line.strip('\n').split(), predicts)
    predicts = np.array(list(predicts))
...
if __name__ == '__main__':
    eval(LResnet50.LResNet50E_IR(is_gray=False), model_path='chkpt_5/CosFace_20_checkpoint.pth')
